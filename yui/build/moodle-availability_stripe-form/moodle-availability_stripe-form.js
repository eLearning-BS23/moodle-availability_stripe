YUI.add('moodle-availability_stripe-form', function (Y, NAME) {

/**
 * JavaScript for form editing date conditions.
 *
 * @module moodle-availability_stripe-form
 */
M.availability_stripe = M.availability_stripe || {};

/**
 * @class M.availability_stripe.form
 * @extends M.core_availability.plugin
 */
M.availability_stripe.form = Y.Object(M.core_availability.plugin);
 ...
 */
M.availability_stripe.form.initInner = function(currencies) {
    this.currencies = currencies;
};

M.availability_stripe.form.getNode = function(json) {
    var selected_string = '';
    ...
    var html = '<div><label>';
    html += M.util.get_string('businessemail', 'availability_stripe');
    html += '<input name="businessemail" type="email" /></label></div>';

    html += '<div><label>';
    html += M.util.get_string('currency', 'availability_stripe');
    ...
    if (!M.availability_stripe.form.addedEvents) {
        M.availability_stripe.form.addedEvents = true;

        var root = Y.one('.availability-field');
        root.delegate('change', function() {
            M.core_availability.form.update();
        }, '.availability_stripe select[name=currency]');
        ...
        root.delegate('change', function() {
                ...
                M.core_availability.form.update();
        }, '.availability_stripe input');
    }

    return node;
};

M.availability_stripe.form.fillValue = function(value, node) {
 ...
 */
M.availability_stripe.form.getValue = function(field, node) {
    // Get field value.
...
M.availability_stripe.form.fillErrors = function(errors, node) {
    var value = {};
    this.fillValue(value, node);

    if (value.businessemail === '') {
        errors.push('availability_stripe:error_businessemail');
    }
